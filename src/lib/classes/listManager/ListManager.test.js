...
describe("ListManager", () => {
  ...
  test("should delete an item successfully", () => {
    ...
    expect(() =>
      listManager.get(idToDel).toThrowError(`Item with ID ${idToDel} not found`)
    );
  });
  ...
  test("should throw an error when deleting a non-existing item", () => {
    ...
    expect(() => listManager.delete(nonExistingItemId)).toThrowError(
      `Item with ID ${nonExistingItemId} not found`
    );
  });
});
